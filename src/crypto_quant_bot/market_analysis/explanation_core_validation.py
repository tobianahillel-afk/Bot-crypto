...
def validate_statements(
    ...
    for statement in statements:
        ...
        template = mapping(templates[template_id], f"templates.{template_id}")
        ...
        if not isinstance(refs, list) or not refs:
            raise ExplanationEvidenceError(f"reason code has no source evidence: {statement['reason_code']}")
...
def validate_reason_set(
    ...
    for reason_value in reasons:
        ...
        if not isinstance(refs, list) or not refs:
            raise ExplanationEvidenceError(f"reason has no source evidence: {code}")
        for reference in refs:
            validate_evidence_ref(mapping(reference, "evidence"), sources)
        codes.append(code)
