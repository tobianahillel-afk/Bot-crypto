...
def validate_precision(raw_timestamp: str, precision: str) -> None:
    digits = PRECISION_DIGITS.get(precision)
    if digits is None:
        raise TimestampGovernanceError("timestamp_precision is unknown")
    match = TIMESTAMP_PATTERN.fullmatch(raw_timestamp)
    if match is None:
        raise TimestampGovernanceError("raw_timestamp must be timezone-aware ISO-8601")
    fraction = match.group(1) or ""
    if len(fraction) != digits:
        raise TimestampGovernanceError("raw_timestamp precision does not match declaration")
