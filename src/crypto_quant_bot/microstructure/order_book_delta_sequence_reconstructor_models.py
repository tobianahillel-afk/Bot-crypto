...
class OrderBookDeltaV1:
    ...
    def __post_init__(self) -> None:
        ...
        if self.market_type != "SPOT":
            raise OrderBookDeltaSequenceValidationError("Lot 39 delta market_type must be SPOT")
        ...
        if not self.bids and not self.asks:
            raise OrderBookDeltaSequenceValidationError("Lot 39 delta must change at least one side")
        ...
        if self.used_for_decision is not False:
            raise OrderBookDeltaSequenceValidationError("Lot 39 delta cannot be decision data")
...
class ReconstructedOrderBookV1:
    ...
    def __post_init__(self) -> None:
        ...
        require_text(self.instrument_id, "instrument_id")
        if self.market_type != "SPOT":
            raise OrderBookDeltaSequenceValidationError("reconstructed market_type must be SPOT")
        validate_causal_times(self.event_time, self.receive_time, self.receive_time)
        ...
        if self.sequence_id <= self.base_sequence_id:
            raise OrderBookDeltaSequenceValidationError("reconstructed sequence must advance")
        require_sha256(self.sequence_anchor, "sequence_anchor")
        if self.synchronization_state != "SYNCED":
            raise OrderBookDeltaSequenceValidationError("only SYNCED books may be published")
        if not self.bids or not self.asks:
            raise OrderBookDeltaSequenceValidationError("reconstructed book sides must be non-empty")
        if tuple(sorted(self.bids, key=lambda level: level.price, reverse=True)) != self.bids:
            raise OrderBookDeltaSequenceValidationError("reconstructed bids must be descending")
        if tuple(sorted(self.asks, key=lambda level: level.price)) != self.asks:
            raise OrderBookDeltaSequenceValidationError("reconstructed asks must be ascending")
        if self.bids[0].price >= self.asks[0].price:
            raise OrderBookDeltaSequenceValidationError("crossed or locked reconstructed book forbidden")
    ...
    def __post_init__(self) -> None:
        ...
        if self.gap_detected:
            if self.synchronization_state != "RESYNC_REQUIRED":
                raise OrderBookDeltaSequenceValidationError("detected gap requires resync")
            if self.observed_sequence is None or self.observed_prev_sequence is None:
                raise OrderBookDeltaSequenceValidationError("detected gap requires observed sequence")
            ...
            if self.synchronization_state != "SYNCED":
                raise OrderBookDeltaSequenceValidationError("no-gap event must remain SYNCED")
            if self.observed_sequence is not None or self.observed_prev_sequence is not None:
                raise OrderBookDeltaSequenceValidationError("no-gap event cannot carry observed gap")
...
class Lot39MetricsV1:
    ...
    def __post_init__(self) -> None:
        ...
        if self.deltas_applied_total > self.deltas_received_total:
            raise OrderBookDeltaSequenceValidationError("applied deltas exceed received deltas")
...
class OrderBookDeltaSequenceReconstructorStateV1:
    ...
    base_snapshot_checksum: str
    delta_fixture_checksum: str
    reconstructed_book: ReconstructedOrderBookV1
    ...
    def __post_init__(self) -> None:
        ...
        if self.validation_state != "VALIDATED_OFFLINE_DELTA_SEQUENCE_RECONSTRUCTION_ONLY":
            raise OrderBookDeltaSequenceValidationError("Lot 39 validation state changed")
        if self.synchronization_state != "SYNCED":
            raise OrderBookDeltaSequenceValidationError("persisted Lot 39 state must be SYNCED")
        require_sha256(self.base_snapshot_checksum, "base_snapshot_checksum")
        require_sha256(self.delta_fixture_checksum, "delta_fixture_checksum")
        if self.reconstructed_book.synchronization_state != self.synchronization_state:
            raise OrderBookDeltaSequenceValidationError("state/book synchronization mismatch")
        if self.sequence_gap_event.gap_detected:
            raise OrderBookDeltaSequenceValidationError("canonical Lot 39 state cannot persist a gap")
        validate_reason_codes(self.reason_codes)
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "base_snapshot_checksum": self.base_snapshot_checksum,
            "delta_fixture_checksum": self.delta_fixture_checksum,
            "reconstructed_book": self.reconstructed_book.to_dict(),
...
class OrderBookDeltaSequenceReconstructorAuditV1:
    ...
    def __post_init__(self) -> None:
        ...
        for value, field in (
            ...
            (self.sequence_gap_event_checksum, "sequence_gap_event_checksum"),
            (self.audit_checksum, "audit_checksum"),
        ):
            require_sha256(value, field)
        if self.validation_state != "VALIDATED_OFFLINE_DELTA_SEQUENCE_RECONSTRUCTION_ONLY":
            raise OrderBookDeltaSequenceValidationError("Lot 39 audit state changed")
        if self.synchronization_state != "SYNCED":
            raise OrderBookDeltaSequenceValidationError("Lot 39 audit synchronization changed")
        validate_lot39_safety(self.safety)
