...
def require_sha256(value: object, field: str) -> str:
    ...
    require(len(text) == 64 and all(ch in "0123456789abcdef" for ch in text), f"{field} must be lowercase sha256")
    return text


def require_git_sha(value: object, field: str) -> str:
    text = require_text(value, field)
    require(len(text) == 40 and all(ch in "0123456789abcdef" for ch in text), f"{field} must be git sha")
    return text


def decimal_from_text(value: object, field: str, *, allow_zero: bool = False) -> Decimal:
    require(isinstance(value, str), f"{field} must be decimal text")
    try:
        number = Decimal(value)
    except InvalidOperation as exc:
        raise TradesAggressorClassificationValidationError(f"{field} invalid decimal") from exc
    require(number.is_finite(), f"{field} must be finite")
    require(number >= 0 if allow_zero else number > 0, f"{field} out of range")
    return number
...
def parse_utc_timestamp(value: object, field: str) -> datetime:
    ...
    try:
        ...
        raise TradesAggressorClassificationValidationError(f"{field} invalid UTC timestamp") from exc
    require(parsed.tzinfo == timezone.utc, f"{field} must be UTC")
    return parsed
...
def validate_causal_times(event_time: str, receive_time: str, generated_at: str) -> None:
    ...
    require(event <= receive <= generated, "causal timestamps require event <= receive <= generated")
...
def validate_run_context(run_id: str, runtime_mode: str, config_version: str, code_commit: str, correlation_id: str) -> None:
