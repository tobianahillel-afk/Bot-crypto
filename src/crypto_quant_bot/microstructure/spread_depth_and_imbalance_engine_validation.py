...
def parse_book_levels(raw: Any, side: str) -> tuple[tuple[Decimal, Decimal], ...]:
    ...
    for index, level in enumerate(raw):
        ...
        quantity = positive_decimal_text(level.get("quantity"), f"{side}[{index}].quantity")
        parsed.append((price, quantity))
    validate_level_order(tuple(parsed), side)
    return tuple(parsed)


def validate_level_order(levels: tuple[tuple[Decimal, Decimal], ...], side: str) -> None:
...
def validate_reference_identity(book: dict[str, Any], integrity: dict[str, Any]) -> None:
    fields = ("source_id", "venue", "instrument_id", "market_type", "sequence_id")
    for field in fields:
        if book.get(field) != integrity.get(field):
            raise Lot41ValidationError(f"Lot 40/book identity mismatch: {field}")
...
def validate_reference_times(
    book: dict[str, Any], integrity: dict[str, Any], decision_time: str, generated_at: str
) -> None:
    ...
    if integrity.get("event_time") != event_time or integrity.get("receive_time") != receive_time:
...
def symmetric_imbalance(bid_depth: Decimal, ask_depth: Decimal) -> tuple[Decimal | None, str]:
