...
def _validate_delta_content(delta: OrderBookDeltaV1) -> None:
    if not delta.bids and not delta.asks:
        raise OrderBookDeltaSequenceValidationError(
            "Lot 39 delta must change at least one side"
        )
    if delta.expected_book_checksum is not None:
        require_sha256(delta.expected_book_checksum, "expected_book_checksum")
    if delta.used_for_decision is not False:
        raise OrderBookDeltaSequenceValidationError(
...
def _validate_gap_event(event: SequenceGapEventV1) -> None:
    ...
    if event.gap_detected:
        _validate_detected_gap(event)
    else:
        _validate_no_gap(event)


def _validate_detected_gap(event: SequenceGapEventV1) -> None:
...
def _validate_no_gap(event: SequenceGapEventV1) -> None:
    observed = (event.observed_sequence, event.observed_prev_sequence)
    if event.synchronization_state != "SYNCED" or observed != (None, None):
        raise OrderBookDeltaSequenceValidationError(
...
def _validate_blocked_state(
    ...
    if sequence_gap_event.synchronization_state != "RESYNC_REQUIRED":
        raise OrderBookDeltaSequenceValidationError("state/gap synchronization mismatch")
...
def _validate_state_outcome(
    ...
    if synchronization_state == "SYNCED":
        _validate_synced_state(validation_state, reconstructed_book, sequence_gap_event)
        return
    _validate_blocked_state(validation_state, reconstructed_book, sequence_gap_event)
...
def _validate_audit_outcome(
    ...
    validate_sync_state(synchronization_state)
    if synchronization_state == "SYNCED":
        _validate_synced_audit(
            validation_state,
            reconstructed_book_checksum,
            sequence_gap_event_checksum,
        )
        return
    _validate_blocked_audit(
        validation_state,
        reconstructed_book_checksum,
        sequence_gap_event_checksum,
    )
