...
def run_lot7_smoke() -> tuple[int, str]:
    sh_path = ROOT / "scripts" / "validate_all_until_lot7.sh"
    text = sh_path.read_text(encoding="utf-8")
    assert "CQB_ORCHESTRATOR_MODE" in text
    assert "LOT 7 ORCHESTRATOR SMOKE: PASS" in text
    ...
    return 0, "LOT 7 ORCHESTRATOR SMOKE: PASS"


def test_validate_all_until_lot7_smoke_executes_without_nested_pytest():
