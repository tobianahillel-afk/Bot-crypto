...
LOT24_PYTHON_SCRIPTS = [
    ...
    ROOT / "scripts" / "diagnose_lot24_required_chain_timing.py",
...
def test_lot24_python_scripts_avoid_forbidden_process_hacks():
    for path in LOT24_PYTHON_SCRIPTS:
        text = path.read_text(encoding="utf-8")
        for token in _forbidden_tokens():
            assert token not in text, f"{path.name} contains forbidden token: {token}"


def test_lot24_diagnostics_use_subprocess_run_only():
...
def test_lot24_exact_chain_covers_lot23_lot24_and_pytest():
    text = (ROOT / "scripts" / "diagnose_exact_chain_until_lot24.py").read_text(encoding="utf-8")
    required_steps = [
