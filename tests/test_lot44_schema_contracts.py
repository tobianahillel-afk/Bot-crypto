...
def test_reference_classified_trades_and_confidence_validate() -> None:
    ...
    jsonschema.validate(
        state.confidence_state.to_dict(),
        _schema("aggressor_confidence_state_v1.schema.json"),
    )
...
def test_reference_state_and_audit_validate() -> None:
    ...
    jsonschema.validate(
        state.to_dict(),
        _schema("trades_aggressor_classification_schema_state_v1.schema.json"),
        resolver=resolver,
    )
    jsonschema.validate(
        audit.to_dict(),
        _schema("trades_aggressor_classification_schema_audit_v1.schema.json"),
    )
...
def test_schemas_do_not_authorize_lot45_or_lot46_outputs() -> None:
    ...
    assert (
        "trade_allowed" not in joined
        or "audit" in joined.lower()
        or "state" in joined.lower()
    )
