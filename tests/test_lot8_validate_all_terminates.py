...
def run_lot8_smoke() -> tuple[int, str]:
    sh_path = ROOT / "scripts" / "validate_all_until_lot8.sh"
    text = sh_path.read_text(encoding="utf-8")
    assert "CQB_ORCHESTRATOR_MODE" in text
    assert "LOT 8 ORCHESTRATOR SMOKE: PASS" in text
    ...
    return 0, "LOT 8 ORCHESTRATOR SMOKE: PASS"


def test_validate_all_until_lot8_smoke_executes_without_nested_pytest():
