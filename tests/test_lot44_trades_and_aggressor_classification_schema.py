...
from crypto_quant_bot.data_governance.market_data_governance_scope_and_source_registry import load_json_object
...
from crypto_quant_bot.microstructure.trades_and_aggressor_classification_schema_models import TimestampedTradeV1
...
def _trade(*, trade_id: str, price: str, event_time: str, receive_time: str) -> TimestampedTradeV1:
...
def test_reference_quote_test_classification_and_conservation() -> None:
    ...
    assert state.metrics.total_volume == state.metrics.buy_volume + state.metrics.sell_volume + state.metrics.unknown_volume
...
def test_confidence_is_descriptive_not_probability_engine() -> None:
    ...
    assert all(item.confidence_version == "lot44-aggressor-confidence-v1" for item in state.classified_trades)
...
def test_tick_rule_only_when_quote_unavailable() -> None:
    state, _ = _reference()
    confidence = state.confidence_state
    previous = _trade(trade_id="prev", price="100", event_time="2026-08-06T19:18:39.000000Z", receive_time="2026-08-06T19:18:39.010000Z")
    up = _trade(trade_id="up", price="101", event_time="2026-08-06T19:18:40.000000Z", receive_time="2026-08-06T19:18:40.010000Z")
    result = classify_trade(up, None, previous, max_quote_age_us=250000, confidence=confidence, tick_rule_fallback=True)
...
def test_zero_tick_without_quote_stays_unknown() -> None:
    ...
    confidence = state.confidence_state
    previous = _trade(trade_id="prev", price="100", event_time="2026-08-06T19:18:39.000000Z", receive_time="2026-08-06T19:18:39.010000Z")
    same = _trade(trade_id="same", price="100", event_time="2026-08-06T19:18:40.000000Z", receive_time="2026-08-06T19:18:40.010000Z")
    result = classify_trade(same, None, previous, max_quote_age_us=250000, confidence=confidence, tick_rule_fallback=True)
...
def test_future_quote_is_never_backfilled_and_does_not_tick_fallback() -> None:
    ...
    trade = _trade(trade_id="future-quote", price="50025.10", event_time="2026-08-06T19:18:39.900000Z", receive_time="2026-08-06T19:18:40.040000Z")
    result = classify_trade(trade, snapshot, None, max_quote_age_us=250000, confidence=state.confidence_state, tick_rule_fallback=True)
    assert result.aggressor_classification == "UNKNOWN"
...
def test_stale_quote_degrades_to_unknown() -> None:
    state, _ = _reference()
    snapshot = _load_snapshot(ROOT, load_json_object(ROOT / CONFIG_PATH))
    trade = _trade(trade_id="stale", price="50025.10", event_time="2026-08-06T19:18:41.000000Z", receive_time="2026-08-06T19:18:41.000000Z")
    result = classify_trade(trade, snapshot, None, max_quote_age_us=250000, confidence=state.confidence_state, tick_rule_fallback=True)
    assert result.aggressor_classification == "UNKNOWN"
...
def test_locked_quote_degrades_to_unknown() -> None:
    state, _ = _reference()
    snapshot = _load_snapshot(ROOT, load_json_object(ROOT / CONFIG_PATH))
    locked = replace(snapshot, venue_state="LOCKED", bids=(OrderBookLevelV1(snapshot.asks[0].price, Decimal("1")), *snapshot.bids[1:]))
    trade = _trade(trade_id="locked", price="50025.10", event_time=snapshot.event_time, receive_time=snapshot.receive_time)
    result = classify_trade(trade, locked, None, max_quote_age_us=250000, confidence=state.confidence_state, tick_rule_fallback=True)
    assert result.aggressor_classification == "UNKNOWN"
...
def test_identity_mismatch_fails_closed() -> None:
    state, _ = _reference()
    snapshot = _load_snapshot(ROOT, load_json_object(ROOT / CONFIG_PATH))
    trade = replace(state.classified_trades[1].trade, instrument_id="ETH-EUR-SPOT")
    with pytest.raises(TradesAggressorClassificationValidationError, match="identity mismatch"):
        classify_trade(trade, snapshot, None, max_quote_age_us=250000, confidence=state.confidence_state, tick_rule_fallback=True)
