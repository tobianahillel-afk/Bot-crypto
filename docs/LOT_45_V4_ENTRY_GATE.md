...
1. the current gate descends from the exact Lot 44 post-merge audit merge;
...
4. the gate branch contains governance artifacts only;
5. no Lot 45 implementation path existed before this gate;
6. Lot 46 remains `PLANNED_LOCKED` and physically absent;
7. safety remains fail-closed with approved size zero and no execution capability.
