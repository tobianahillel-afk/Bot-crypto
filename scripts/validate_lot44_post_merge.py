...
def validate_git_chain() -> None:
    ...
    for older, newer, label in (
        ...
        (IMPLEMENTATION_MERGE, head, "implementation-merge/audit-head"),
    ):
        require(is_ancestor(older, newer), f"Lot44 ancestry failed: {label}")

    merge_parents = set(git("show", "-s", "--format=%P", IMPLEMENTATION_MERGE).split())
    require(FINAL_PR_HEAD in merge_parents, "implementation merge does not contain final PR head")
    require(len(merge_parents) >= 2, "implementation merge must remain a merge commit")
    ...
    audit_paths = changed_paths(IMPLEMENTATION_MERGE, head)
    require(
        audit_paths <= AUDIT_BRANCH_PATHS,
        f"unexpected post-merge audit branch paths: {sorted(audit_paths - AUDIT_BRANCH_PATHS)}",
    )
...
def validate_immutability() -> None:
    ...
    for path in SOURCE_IMMUTABLE_PATHS:
        require(
            not changed_paths(SOURCE_HEAD, head).intersection({path}),
            f"Lot44 source/test/doc drift after source freeze: {path}",
        )
    require(
...
def validate() -> dict[str, Any]:
    ...
    payload: dict[str, Any] = {
        "schema_version": "lot44-post-merge-audit-v1",
        "status": "PASS",
