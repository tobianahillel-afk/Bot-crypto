...
def validate_file(path: Path) -> None:
    ...
    for marker in FORBIDDEN_TEXT:
        if marker in text:
            raise RuntimeError(f"LOT42_FORBIDDEN_TEXT:{path}:{marker}")
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.Import | ast.ImportFrom):
            for imported in _import_names(node):
                if any(
                    ...
                    raise RuntimeError(f"LOT42_FORBIDDEN_IMPORT:{path}:{imported}")
        if isinstance(node, ast.Call):
            ...
            if name in FORBIDDEN_CALL_NAMES:
                raise RuntimeError(f"LOT42_FORBIDDEN_CALL:{path}:{name}")
...
def main() -> int:
    try:
        for relative in FILES:
            path = ROOT / relative
            if not path.exists():
                raise RuntimeError(f"LOT42_REQUIRED_FILE_MISSING:{relative}")
            validate_file(path)
        print("LOT42_NO_CONNECTIVITY_VALIDATED")
    except (OSError, SyntaxError, RuntimeError) as exc:
        print(f"LOT42 NO CONNECTIVITY: FAIL\n{exc}", file=sys.stderr)
        return 1
    return 0
