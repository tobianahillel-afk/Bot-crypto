...
REQUIRED_FILES = [
    ...
    "src/crypto_quant_bot/market_analysis/confluence_models.py",
...
EXPECTED_CATALOG_IDS = {
    "volatility_regime_confluence_lot25",
...
def fail(message: str) -> int:
    print("LOT 25 VALIDATION: FAIL", flush=True)
...
def main() -> int:
    for relative in REQUIRED_FILES:
        if not (ROOT / relative).exists():
            return fail(f"missing Lot 25 artifact: {relative}")

    registry_path = ROOT / LOT25_OUTPUT_PATH
    timeframes_path = ROOT / LOT25_TIMEFRAMES_OUTPUT_PATH
    report_path = ROOT / LOT25_REPORT_OUTPUT_PATH
    validation_report_path = ROOT / LOT25_VALIDATION_REPORT_PATH
    archive_path = ROOT / ARCHIVE_OUTPUT_PATH

    snapshot = load_json(registry_path)
    if not isinstance(snapshot, dict):
        return fail("volatility regime confluence payload must be a JSON object")
    ...
    if snapshot.get("vrc_timeframes") != ["5m", "15m"]:
        return fail("vrc_timeframes must be ['5m', '15m']")
    ...
    if not validate_checksum(str(snapshot.get("vrc_checksum", ""))):
        return fail("vrc_checksum missing or invalid")
    if build_vrc_checksum(snapshot) != snapshot.get("vrc_checksum"):
        return fail("vrc_checksum mismatch")
    ...
    for key, value in VRC_INVARIANTS.items():
        if key in snapshot and snapshot.get(key) != value:
            return fail(f"invariant mismatch for {key}: {snapshot.get(key)}")

    block_reasons = snapshot.get("vrc_block_reasons")
    if not isinstance(block_reasons, list):
        return fail("vrc_block_reasons must be a list")
    if set(DEFAULT_VRC_BLOCK_REASONS) - set(block_reasons):
        return fail("missing required vrc_block_reasons")

    allowed_volatility_states = set(ALLOWED_VOLATILITY_STATES)
    allowed_regime_states = set(ALLOWED_REGIME_STATES)
    allowed_confluence_states = set(ALLOWED_CONFLUENCE_STATES)
    allowed_combined_states = set(ALLOWED_VRC_COMBINED_STATES)

    for row in timeframe_rows:
        summary = VolatilityRegimeConfluenceTimeframeSummary(**row)
        if summary.volatility_state not in allowed_volatility_states:
            return fail(f"{summary.timeframe} volatility_state invalid: {summary.volatility_state}")
        if summary.regime_state not in allowed_regime_states:
            return fail(f"{summary.timeframe} regime_state invalid: {summary.regime_state}")
        if summary.confluence_state not in allowed_confluence_states:
            return fail(f"{summary.timeframe} confluence_state invalid: {summary.confluence_state}")
        ...
        if not 0.0 <= float(summary.volatility_context_score) <= 1.0:
            return fail(f"{summary.timeframe} volatility_context_score out of bounds")
        if not 0.0 <= float(summary.regime_context_score) <= 1.0:
            return fail(f"{summary.timeframe} regime_context_score out of bounds")
        if not 0.0 <= float(summary.confluence_context_score) <= 1.0:
            return fail(f"{summary.timeframe} confluence_context_score out of bounds")
        if not 0.0 <= float(summary.combined_context_score) <= 1.0:
            return fail(f"{summary.timeframe} combined_context_score out of bounds")
        if not isinstance(summary.confluence_components, dict):
            return fail(f"{summary.timeframe} confluence_components must be a mapping")
    ...
    if snapshot.get("volatility_state") not in allowed_volatility_states:
        return fail("volatility_state invalid")
    if snapshot.get("regime_state") not in allowed_regime_states:
        return fail("regime_state invalid")
    if snapshot.get("confluence_state") not in allowed_confluence_states:
        return fail("confluence_state invalid")
    ...
    if lot21_scope.get("source_v1_archive_sha256") != observed_archive_checksum:
        return fail("Lot 25 modified the frozen archive checksum")
    if lot20_closure.get("archive_sha256") != observed_archive_checksum:
        return fail("Lot 25 modified the Lot 20 archive checksum")
    if lot21_scope.get("source_v1_archive_size_bytes") != archive_path.stat().st_size:
        return fail("Lot 25 modified the frozen archive size")

    for path in [
        registry_path,
        timeframes_path,
        report_path,
        ROOT / LOT25_OVERVIEW_DOC_PATH,
        ROOT / LOT25_ACCEPTANCE_DOC_PATH,
    ]:
        message = validate_output_text(path)
        if message:
            return fail(message)

    wrapper_text = read_text_limited(ROOT / "scripts/validate_all_until_lot25.py")
    chain_text = read_text_limited(ROOT / "scripts/run_required_chain_until_lot25.sh")
    exact_text = read_text_limited(ROOT / "scripts/diagnose_exact_chain_until_lot25.py")
    ...
    if legacy_lot20_step in wrapper_text or legacy_lot20_step in chain_text or legacy_lot20_step in exact_text:
        return fail(f"Lot 25 chains must not replay {legacy_lot20_step}")
    ...
    if set(EXPECTED_CATALOG_IDS) - set(catalog_ids):
        return fail("dataset_catalog missing Lot 25 entries")

    snapshot_object = VolatilityRegimeConfluenceResult(
        **{
            **snapshot,
            "timeframe_summaries": [VolatilityRegimeConfluenceTimeframeSummary(**row) for row in timeframe_rows],
            "vrc_checks": snapshot.get("vrc_checks", []),
        }
    )
    write_vrc_validation_report(validation_report_path, snapshot=snapshot_object)
    validation_message = validate_output_text(validation_report_path)
    if validation_message:
        return fail(validation_message)

    print("LOT 25 VALIDATION: PASS", flush=True)
    return 0
