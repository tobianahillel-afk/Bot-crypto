...
echo "=== Crypto Quant Bot Lot 24 required chain ==="
require_file scripts/run_required_chain_until_lot23.sh
...
require_file scripts/run_lot24_trend_range_momentum.py
require_file scripts/validate_lot24.py

timeout 240s bash scripts/run_required_chain_until_lot23.sh
...
timeout 60s python scripts/run_lot24_trend_range_momentum.py
timeout 60s python scripts/validate_lot24.py

echo "LOT 24 REQUIRED CHAIN: PASS"
exit 0
