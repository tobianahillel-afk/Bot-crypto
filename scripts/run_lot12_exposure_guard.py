...
from crypto_quant_bot.exposure.io import load_jsonl, write_jsonl, write_report
...
OUTPUTS = {
    ...
    "report": ROOT / "reports" / "lot_12_exposure_guard_report.md",
}


def fail(message: str) -> int:
    print("LOT 12 EXPOSURE GUARD: FAIL", flush=True)
...
def upsert_catalog(path: Path, dataset_id: str, timeframe: str, row_count: int, start_timestamp: str, end_timestamp: str) -> None:
    DatasetCatalog(ROOT / "data" / "audit" / "dataset_catalog.json").upsert(
        DatasetMetadata(
            ...
            timeframe=timeframe,
            layer="audit",
            data_version="lot12_v0",
            start_timestamp=start_timestamp,
            end_timestamp=end_timestamp,
            row_count=row_count,
            checksum=sha256_file(path),
            source="lot12_exposure_guard_v0",
            lineage_id=f"lot12_{dataset_id}_lineage",
            quality_flag="valid",
            validation_status="validated_lot12",
...
def main() -> int:
    for mapping in [MARKET_STATE_INPUTS, COST_INPUTS, RISK_INPUTS]:
        for path in mapping.values():
            if not path.exists():
                return fail(f"missing input: {path}")
    guard = ExposureGuard(policy_version="lot12_exposure_guard_v0")
    counts: dict[str, int] = {}
    total = 0
    for timeframe in ["5m", "15m"]:
        market_state_rows = load_jsonl(MARKET_STATE_INPUTS[timeframe])
        cost_rows = load_jsonl(COST_INPUTS[timeframe])
        risk_rows = load_jsonl(RISK_INPUTS[timeframe])
        ...
        snapshots = guard.build_snapshots(
            timeframe,
    ...
    print("LOT 12 EXPOSURE GUARD: PASS", flush=True)
    return 0
