...
def main() -> int:
    ...
    for label, command, timeout_seconds in STEPS:
        ...
        if rc != 0:
            print(f"DIAGNOSE LOT5 FD LINGERING OWNER: FAIL first_step={label} rc={rc}", flush=True)
            return rc
        ...
        if offenders:
            ...
            print(f"DIAGNOSE LOT5 FD LINGERING OWNER: FAIL owner={label}", flush=True)
            flush_streams()
            return 1
        print(f"NO_LINGERING_AFTER:{label}", flush=True)
    print("DIAGNOSE LOT5 FD LINGERING OWNER: PASS", flush=True)
    flush_streams()
    return 0
