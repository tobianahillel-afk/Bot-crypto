...
FORBIDDEN_IMPORT_ROOTS = {
    "aiohttp", "boto3", "ccxt", "http.client", "httpx", "requests", "socket",
    "urllib" + ".request", "urllib3", "websocket", "websockets",
}
FORBIDDEN_CALL_NAMES = {"connect", "create_connection", "getaddrinfo", "request", "socket", "urlopen"}
FORBIDDEN_TEXT = (
    "http" + "://", "https" + "://", "wss" + "://", "API_KEY", "API_SECRET",
...
def validate_file(path: Path) -> None:
    ...
    for marker in FORBIDDEN_TEXT:
        if marker in text:
            raise RuntimeError(f"LOT41_FORBIDDEN_TEXT:{path}:{marker}")
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.Import | ast.ImportFrom):
            ...
            if any(imported == root or imported.startswith(root + ".") for root in FORBIDDEN_IMPORT_ROOTS):
                raise RuntimeError(f"LOT41_FORBIDDEN_IMPORT:{path}:{imported}")
        if isinstance(node, ast.Call):
            ...
            if name in FORBIDDEN_CALL_NAMES:
                raise RuntimeError(f"LOT41_FORBIDDEN_CALL:{path}:{name}")
...
def main() -> int:
    try:
        for relative in FILES:
            path = ROOT / relative
            if not path.exists():
                raise RuntimeError(f"LOT41_REQUIRED_FILE_MISSING:{relative}")
            validate_file(path)
        print("LOT41_NO_CONNECTIVITY_VALIDATED")
    except (OSError, SyntaxError, RuntimeError) as exc:
        print(f"LOT41 NO CONNECTIVITY: FAIL\n{exc}", file=sys.stderr)
        return 1
    return 0
