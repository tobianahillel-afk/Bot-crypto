...
def main() -> int:
    print("BEFORE:exact_chain_lot21", flush=True)
    print("COMMAND:exact_chain_lot21:bash -lc <exact Lot 0 to Lot 21 chain>", flush=True)
    print("EXPECTED_MARKER:exact_chain_lot21:EXACT_CHAIN_LOT21_DONE", flush=True)
    ...
    try:
        ...
        print(f"TIMEOUT:exact_chain_lot21:rc=124:duration_seconds={duration:.3f}", flush=True)
        print("DIAGNOSE EXACT CHAIN LOT21: FAIL", flush=True)
        return 124
    duration = time.monotonic() - started
    rc = int(result.returncode)
    print(f"AFTER:exact_chain_lot21:rc={rc}:duration_seconds={duration:.3f}", flush=True)
    if rc != 0:
        print("DIAGNOSE EXACT CHAIN LOT21: FAIL", flush=True)
        return rc
    print("DIAGNOSE EXACT CHAIN LOT21: PASS", flush=True)
    return 0
