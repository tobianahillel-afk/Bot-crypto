...
def validate_file(path: Path) -> None:
    ...
    for marker in FORBIDDEN_TEXT:
        if marker in text:
            raise RuntimeError(f"LOT40_FORBIDDEN_TEXT:{path}:{marker}")
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.Import | ast.ImportFrom):
            ...
            if any(imported == root or imported.startswith(root + ".") for root in FORBIDDEN_IMPORT_ROOTS):
                raise RuntimeError(f"LOT40_FORBIDDEN_IMPORT:{path}:{imported}")
        if isinstance(node, ast.Call):
            ...
            if name in FORBIDDEN_CALL_NAMES:
                raise RuntimeError(f"LOT40_FORBIDDEN_CALL:{path}:{name}")
...
def main() -> int:
    try:
        for relative in FILES:
            path = ROOT / relative
            if not path.exists():
                raise RuntimeError(f"LOT40_REQUIRED_FILE_MISSING:{relative}")
            validate_file(path)
        print("LOT40_NO_CONNECTIVITY_VALIDATED")
    except (OSError, SyntaxError, RuntimeError) as exc:
        print(f"LOT40 NO CONNECTIVITY: FAIL\n{exc}", file=sys.stderr)
        return 1
    return 0
