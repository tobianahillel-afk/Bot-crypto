...
def main() -> int:
    print("BEFORE:exact_chain_lot16", flush=True)
    print("COMMAND:exact_chain_lot16:bash -lc <exact Lot 0 to Lot 16 chain + Lot 17 refresh>", flush=True)
    print("EXPECTED_MARKER:exact_chain_lot16:EXACT_CHAIN_LOT16_DONE", flush=True)
    ...
    try:
        ...
        print(f"TIMEOUT:exact_chain_lot16:rc=124:duration_seconds={duration:.3f}", flush=True)
        print("DIAGNOSE EXACT CHAIN LOT16: FAIL", flush=True)
        return 124
    duration = time.monotonic() - started
    rc = int(result.returncode)
    print(f"AFTER:exact_chain_lot16:rc={rc}:duration_seconds={duration:.3f}", flush=True)
    if rc != 0:
        print("DIAGNOSE EXACT CHAIN LOT16: FAIL", flush=True)
        return rc
    print("DIAGNOSE EXACT CHAIN LOT16: PASS", flush=True)
    return 0
