...
def main() -> int:
    print("BEFORE:exact_chain_lot23", flush=True)
    print("COMMAND:exact_chain_lot23:bash -lc <exact Lot 0 to Lot 23 chain>", flush=True)
    print("EXPECTED_MARKER:exact_chain_lot23:EXACT_CHAIN_LOT23_DONE", flush=True)
    ...
    try:
        ...
        print(f"TIMEOUT:exact_chain_lot23:rc=124:duration_seconds={duration:.3f}", flush=True)
        print("DIAGNOSE EXACT CHAIN LOT23: FAIL", flush=True)
        return 124
    duration = time.monotonic() - started
    rc = int(result.returncode)
    print(f"AFTER:exact_chain_lot23:rc={rc}:duration_seconds={duration:.3f}", flush=True)
    if rc != 0:
        print("DIAGNOSE EXACT CHAIN LOT23: FAIL", flush=True)
        return rc
    print("DIAGNOSE EXACT CHAIN LOT23: PASS", flush=True)
    return 0
