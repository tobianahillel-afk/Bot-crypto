...
from crypto_quant_bot.decision.io import load_jsonl, write_jsonl, write_report
...
OUTPUTS = {
    ...
    "report": ROOT / "reports" / "lot_14_decision_firewall_report.md",
}


def fail(message: str) -> int:
    print("LOT 14 DECISION FIREWALL: FAIL", flush=True)
...
def upsert_catalog(path: Path, dataset_id: str, timeframe: str, row_count: int, start_timestamp: str, end_timestamp: str) -> None:
    DatasetCatalog(ROOT / "data" / "audit" / "dataset_catalog.json").upsert(
        DatasetMetadata(
            ...
            timeframe=timeframe,
            layer="audit",
            data_version="lot14_v0",
            start_timestamp=start_timestamp,
            end_timestamp=end_timestamp,
            row_count=row_count,
            checksum=sha256_file(path),
            source="lot14_decision_firewall_v0",
            lineage_id=f"lot14_{dataset_id}_lineage",
            quality_flag="valid",
            validation_status="validated_lot14",
...
def main() -> int:
    for mapping in [MARKET_STATE_INPUTS, COST_INPUTS, RISK_INPUTS, EXPOSURE_INPUTS, PORTFOLIO_INPUTS]:
        for path in mapping.values():
            if not path.exists():
                return fail(f"missing input: {path}")
    firewall = DecisionFirewall(policy_version="lot14_decision_firewall_v0")
    counts: dict[str, int] = {}
    total = 0
    for timeframe in ["5m", "15m"]:
        market_state_rows = load_jsonl(MARKET_STATE_INPUTS[timeframe], max_lines=EXPECTED_COUNTS[timeframe])
        ...
        portfolio_rows = load_jsonl(PORTFOLIO_INPUTS[timeframe], max_lines=EXPECTED_COUNTS[timeframe])
        if len(market_state_rows) != EXPECTED_COUNTS[timeframe]:
            return fail(f"unexpected Lot 7 documentary count for {timeframe}: {len(market_state_rows)}")
        ...
        if len(portfolio_rows) != EXPECTED_COUNTS[timeframe]:
            return fail(f"unexpected Lot 13 documentary count for {timeframe}: {len(portfolio_rows)}")
        snapshots = firewall.build_snapshots(
            timeframe,
            ...
            source_artifacts=[
                f"btc_eur_{timeframe}_market_state_lot7",
        ...
        upsert_catalog(
            OUTPUTS[timeframe],
            f"final_decision_firewall_lot14_{timeframe}",
            timeframe,
            len(snapshots),
            start_timestamp,
            end_timestamp,
    ...
    print("LOT 14 DECISION FIREWALL: PASS", flush=True)
    return 0
