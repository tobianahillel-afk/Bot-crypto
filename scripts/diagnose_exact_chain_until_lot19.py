...
def main() -> int:
    print("BEFORE:exact_chain_lot19", flush=True)
    print("COMMAND:exact_chain_lot19:bash -lc <exact Lot 0 to Lot 19 chain>", flush=True)
    print("EXPECTED_MARKER:exact_chain_lot19:EXACT_CHAIN_LOT19_DONE", flush=True)
    ...
    try:
        ...
        print(f"TIMEOUT:exact_chain_lot19:rc=124:duration_seconds={duration:.3f}", flush=True)
        print("DIAGNOSE EXACT CHAIN LOT19: FAIL", flush=True)
        return 124
    duration = time.monotonic() - started
    rc = int(result.returncode)
    print(f"AFTER:exact_chain_lot19:rc={rc}:duration_seconds={duration:.3f}", flush=True)
    if rc != 0:
        print("DIAGNOSE EXACT CHAIN LOT19: FAIL", flush=True)
        return rc
    print("DIAGNOSE EXACT CHAIN LOT19: PASS", flush=True)
    return 0
