...
STEPS: list[tuple[str, list[str], int]] = [
    ...
    ("run_lot11_risk_engine", ["python", "scripts/run_lot11_risk_engine.py"], 60),
    ("validate_lot11", ["python", "scripts/validate_lot11.py"], 60),
    ("run_lot12_exposure_guard", ["python", "scripts/run_lot12_exposure_guard.py"], 60),
    ("validate_lot12", ["python", "scripts/validate_lot12.py"], 60),
...
def main() -> int:
    for label, command, timeout_seconds in STEPS:
        ...
        if rc != 0:
            print(f"DIAGNOSE EXACT CHAIN LOT12: FAIL first_step={label} rc={rc}", flush=True)
            return rc
    print("DIAGNOSE EXACT CHAIN LOT12: PASS", flush=True)
    flush_streams()
    return 0
