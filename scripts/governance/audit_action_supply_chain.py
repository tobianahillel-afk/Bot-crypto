...
def validate_policy(policy: dict[str, Any]) -> None:
    ...
    if policy.get("semantics") != "INVENTORY_LEGACY_DEBT_BLOCK_CHANGED_USES_LINES":
    ...
    if changed.get("changed_head_lines_only") is not True:
        raise ActionSupplyChainError("changed mode must gate added/modified head lines only")
...
def parse_changed_head_lines(diff_text: str) -> dict[str, set[int]]:
    """Return 1-based added/modified HEAD line numbers from a unified=0 Git diff."""
...
def _git_changed_head_lines(
    ...
    lines = parse_changed_head_lines(proc.stdout)
    extensions = set(policy["workflow_extensions"])
    safe: dict[str, set[int]] = {}
    for path, values in lines.items():
        rel = Path(path)
        if rel.is_absolute() or ".." in rel.parts:
            raise ActionSupplyChainError(f"unsafe changed path in diff: {path!r}")
        full = ROOT / rel
        if full.suffix.lower() in extensions and full.is_file():
            safe[rel.as_posix()] = set(values)
    return safe
...
def changed_gate(
    ...
    changed_records = [
        record
        for record in result["records"]
        if record["line"] in changed_head_lines.get(record["file"], set())
    ]
    violations: list[dict[str, Any]] = []
    ...
    narrowed["records"] = changed_records
    violations.extend(unapproved_pins(narrowed, registry))
    return violations
...
def main() -> int:
    ...
    try:
        ...
        if args.mode == "inventory":
            ...
            files = _git_changed_files(args.base or "", args.head, policy)
            changed_head_lines = _git_changed_head_lines(args.base or "", args.head, policy)
        result = audit(files, policy)
        if changed_head_lines is not None:
            result["changed_head_lines"] = {
                path: sorted(lines) for path, lines in sorted(changed_head_lines.items())
            }
        ...
        if args.mode == "changed":
            if changed_head_lines is None:
                raise ActionSupplyChainError("changed-head-line evidence missing")
            blocked = changed_gate(result, registry, changed_head_lines)
            if blocked:
                for item in blocked:
