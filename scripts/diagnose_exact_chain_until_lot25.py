...
python scripts/run_lot24_trend_range_momentum.py &&
python scripts/validate_lot24.py &&
...
def main() -> int:
    print("BEFORE:exact_chain_lot25", flush=True)
    print("COMMAND:exact_chain_lot25:bash -lc <exact Lot 0 to Lot 25 chain>", flush=True)
    print("EXPECTED_MARKER:exact_chain_lot25:EXACT_CHAIN_LOT25_DONE", flush=True)
    ...
    try:
        ...
        print(f"TIMEOUT:exact_chain_lot25:rc=124:duration_seconds={duration:.3f}", flush=True)
        print("DIAGNOSE EXACT CHAIN LOT25: FAIL", flush=True)
        return 124
    duration = time.monotonic() - started
    rc = int(result.returncode)
    print(f"AFTER:exact_chain_lot25:rc={rc}:duration_seconds={duration:.3f}", flush=True)
    if rc != 0:
        print("DIAGNOSE EXACT CHAIN LOT25: FAIL", flush=True)
        return rc
    print("DIAGNOSE EXACT CHAIN LOT25: PASS", flush=True)
    return 0
